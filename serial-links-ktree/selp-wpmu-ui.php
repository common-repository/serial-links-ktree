...
// Handle the updating of options
	if( isset($_POST['info_update']) ) {
		...
		if ( $updated_options['reset'] == "true" ) {
			...
			echo '<div id="message" class="updated fade"><p><strong>' . __('Serial Links Settings reset to defaults.') . '</strong></p></div>';
		} else {
		// Otherwise, update the options
		update_option( 'selp_plugin_settings', $updated_options);
		echo '<div id="message" class="updated fade"><p><strong>' . __('Serial Links Settings updated and saved.') . '</strong></p></div>';
		}
	}
...
<div class="wrap" id="selpstyle">
	...
	<form method="post">
		...
		<div class="selpinfo">
			<p><em>You are using Serial Links plugin version <?php echo selp_VER; ?> for Wordpress Mu.</em></p>
			<p><?php _e('What does this plugin do?', selp_DOMAIN); ?></p>
			<ul>
				<li>Allows you to assign post links to a Serial, using custom fields, and then displays a list of all post links assigned to the same Serial in your single post link page (usually single.php or index.php).</li>
		...
		<div class="selpinfo">
			...
			<ul>
			    ...
			    <li>In Dasboard &gt;&gt;Links &gt;&gt; Click on Add New. Here at down you would be seeing <strong>Advanced, </strong>in that <strong>Notes</strong> <strong>Text Area</strong> you have to give the same Value as you enter in <strong>Custom Field Valu</strong>e<br />
		      </li>
			</ul>
		...
		<fieldset name="serialposts" class="options">
			...
			<label for="selp-list_current">
				...
				<?php _e('Include current post in list of Serial Links') ?></label>
			<p class="selpsmall"><?php _e('Check the box if you want to include the currently viewed post in the list of Serial Links.'); ?> <em><?php _e('Default is CHECKED.', selp_DOMAIN); ?></em></p>
			...
			<p class="selpsmall"><?php _e('If you have checked "Include current post in list of Serial Links", you may check this box if you want the currently viewed post to be shown as a link.'); ?> <em><?php _e('Default is UNCHECKED.', selp_DOMAIN); ?></em></p>
			</div>
	...
	<div class="selpcredits">
		...
		<p>With acknowledgements to<a href="http://www.studiograsshopper.ch/">Ade Walker</a>&nbsp; whose original code idea inspired this plugin.</p> 
		<p>Serial Posts plugin for Wordpress and Wordpress Mu by <a href="http://wordpress.ktree.com/">Ramana Raju.S </a>&nbsp;&nbsp;&nbsp;<strong>Version: <?php echo selp_VER; ?></strong></p>      
